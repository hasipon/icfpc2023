...
fn coreSolveGaishu(problem:&Problem) -> (f64, Vec<Point>, Vec<f64>) {
...
fn coreSolveHeat(problem:&Problem, heat:Vec<(f64,f64)>, index:i32) -> (f64, Vec<Point>, Vec<f64>) {
    let mut cache = CacheState::new(&problem);
    let mut rng: rand::rngs::StdRng = rand::SeedableRng::seed_from_u64(3);
    let mut placements = Vec::new();
    let mut volume = Vec::new();
...
fn solve(index:&str, timestamp:i64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let heatStr:String = fs::read_to_string(format!("../ueno4/{}.csv", index))?;
    ...
    {
        let (score, placements, volumes) = coreSolveHeat(&problem, heat, index);
        eprintln!("heat {}", score);
        if score > bestScore {
            bestScore = score;
    ...
    {
        let (score, placements, volumes) = coreSolveGaishu(&problem);
