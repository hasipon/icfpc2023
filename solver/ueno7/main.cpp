...
int main() {
    ...
    vector<vector<double>> popular(
            problem.attendees[0].tastes.size(),
            vector<double>(problem.attendees.size())
    );

    vector<int> tasteCount(problem.attendees[0].tastes.size());

    for (int a = 0; a < problem.attendees.size(); a++) {
        for (int t = 0; t < problem.attendees[a].tastes.size(); t++) {
            popular[t][a] = problem.attendees[a].tastes[t];
        }
    }

    for (const auto &musician:problem.musicians) {
        tasteCount[musician]++;
    }

    for (int t = 0; t < problem.attendees[0].tastes.size(); t++) {
       auto [mininum, maximum] = std::minmax_element(popular[t].begin(), popular[t].end());
       auto sum = accumulate(popular[t].begin(), popular[t].end(), 0.);
       cout << "taste[" << t << "] min = " << *mininum << " max = " << *maximum << " mean = " << sum / tasteCount[t] << " size = " << tasteCount[t] << endl;
    }
