...
fn solve(index:u32, timestamp:i64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..18
    { 
        let power = (problem.stage_width + problem.stage_height) / 0.5 / (3.0 + i as f64).powf(1.2);
        println!("{}: {}", index, eval(&problem, &placements));
        yama(x, y, h, w, &problem, &mut placements, power, &mut rng);
        separate_placements(x, y, w, h, &mut placements, &mut rng);
        separate_placements(x, y, w, h, &mut placements, &mut rng);
        separate_placements(x, y, w, h, &mut placements, &mut rng);
        if i % 3 == 1 {
            println!("b {}: {}", index, eval(&problem, &placements));
            try_swap(&problem, &mut placements);
            println!("f {}: {}", index, eval(&problem, &placements));
        }
    }
    ...
    fs::write(
        format!("../../solutions/{}-shohei3.json", index), 
        &answer_string
    )?;
    Ok(())
}
...
fn try_swap(problem:&Problem, placements:&mut Vec<Point>) {
    for i in 0..placements.len()
    {
        for j in i + 1..placements.len()
        {
