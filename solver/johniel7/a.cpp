...
int main(int argc, char** argv) {
    ...
    pair<bool, lli> bestScore = make_pair(false, -1);
    vector<pair<double, double>> placements;
    for (int _ = 0; _ < 200; ++_) {
      vector<pair<double, double>> ps;
      while (ps.size() < problem.musicians.size()) {
        pair<double ,double> p = makeStart(problem, ps);
        ps.push_back(p);
      }
      auto score = calcScore(problem, ps);
      cerr << _ << "th: bestScore=" << bestScore << " v.s. " << score << endl;
      if (bestScore < score) {
        placements = ps;
        bestScore = score;
      }
