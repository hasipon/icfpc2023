use std::{vec, cmp::Ordering, collections::{HashMap, HashSet}};
...
pub fn s_eval_placement(problem:&Problem, placements:&Vec<Point>, index:usize, total:bool, cache:&mut SwapState) -> (f64, f64) {
    ...
    {
        cache.placement_sights[index] = Option::Some(resolve_sight(problem, placements, index, center));
    }
...
fn resolve_sight(problem:&Problem, placements:&Vec<Point>, index:usize, center:Point) -> Vec<Sight>
{    
...
pub fn init_active_attendees(problem:&Problem, placements:&Vec<Point>)-> HashSet<usize> {
    let mut result = HashSet::new();
    for (i, center) in placements.iter().enumerate()
    {
        for sight in resolve_sight(problem, placements, i, *center)
        {
            result.insert(sight.attendee);
        }
    }
    result
}
