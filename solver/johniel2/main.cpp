...
bool isBetterIfSwapped(const Problem& problem, const vector<pair<double, double>>& placements, const int i, const int j) {
  double score = 0;
  for (auto& a : problem.attendees) {
    auto [x, y] = placements[i];
    for (unsigned k = 0; k < placements.size(); k++) if (i != k && j != k) {
        auto [x2, y2] = placements[k];
        if (isBlocked(a.x, a.y, x, y, x2, y2)) {
          goto next1;
        }
      }
    {
      auto d2 = (a.x - x) * (a.x - x) + (a.y - y) * (a.y - y);
      score -= (long long)ceil(1000000 * a.tastes[problem.musicians[i]] / d2); // NOLINT(cppcoreguidelines-narrowing-conversions)
      score += (long long)ceil(1000000 * a.tastes[problem.musicians[j]] / d2); // NOLINT(cppcoreguidelines-narrowing-conversions)
    }
    next1:;
  }
  for (auto& a : problem.attendees) {
...
vector<pair<double, double>> solve(const Problem& problem) {
  ...
  auto best = calcScore(problem, placements);
  for (int _ = 0; _ < 10000; ++_) {
    const int i = rand() % placements.size();
    const int j = rand() % placements.size();
    if (i == j && isBetterIfSwapped(problem, placements, i, j)) {
