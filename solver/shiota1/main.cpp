...
pair<bool, long long> calcScore(const Problem& problem, vector<pair<double, double>> placements) {
    ...
    for (auto& a : problem.attendees) {
        ...
        for(int i = 0; i<placements.size(); i++ ) {
            complex<double> p(placements[i].first, placements[i].second);
            // ミュージシャンの偏角を求める
        ...
        for(int oi = 0; oi<order.size() * 2; oi++){
            ...
            if(!now.isBegin){
                ...
                active.erase(i);
                while(!activeQ.empty() && active.find(activeQ.top().second) == active.end()){
                    activeQ.pop();
                }
                continue;
            }
