...
int main(int argc, char** argv) {
    ...
    {
        auto x = problem.stageLeft + 10;
        auto y = problem.stageBottom + 10;
        while (x <= problem.stageLeft + problem.stageWidth - 10) {
            placements.emplace_back(x, y);
            x += 10;
        }
        x = problem.stageLeft + problem.stageWidth - 10;
        for (;;) {
            if (checkPlacements(x, y, placements)) break;
            ++ y;
        }
        while (y <= problem.stageBottom + problem.stageHeight - 10) {
            placements.emplace_back(x, y);
            y += 10;
        }
        y = problem.stageBottom + problem.stageHeight - 10;
        for (;;) {
            if (checkPlacements(x, y, placements)) break;
            -- x;
        }
        while (x >= problem.stageLeft + 10) {
            placements.emplace_back(x, y);
            x -= 10;
        }
        ...
        while (y >= problem.stageBottom + 10) {
            if (!checkPlacements(x, y, placements)) break;
            placements.emplace_back(x, y);
            y -= 10;
        }
    ...
    if (problem.musicians.size() < placements.size()) {
        cerr << "few musicians " << problem.musicians.size() << " < " << placements.size() << endl;
        return 1;
    }
